...
task main()
{
	...
	nMotorPIDSpeedCtrl[ARM] = mtrSpeedReg;
		...
		if (joystick.joy2_Buttons & button2){ //A
			...
			if ((((armVal - startingArmPos)*0.2+startingWristPos)-wristVal)>25) {
				motor[WRIST] = 20;
			}
			else if ((((armVal - startingArmPos)*0.2+startingWristPos)-wristVal)<-25){
				motor[WRIST] = -20;
			}
			else{
				motor[WRIST]=0;
		...
		else if (joystick.joy2_Buttons & button1){ //X

			if ((((armVal - groundArmPos)*0.2+groundWristPos)-wristVal)>25) {
				motor[WRIST] = 20;
			}
			else if ((((armVal - groundArmPos)*0.2+groundWristPos)-wristVal)<-25){
				motor[WRIST] = -20;
			}
			else{
				motor[WRIST]=0;
