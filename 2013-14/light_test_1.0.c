...
task main()
{
	...
	while (LSvalNorm(LEGOLS)<35 && LSvalNorm(LEGOLS2)<35) {
		drive(E,30,0);
	}
	//drive(W,30,100);
	halt();
	if (LSvalNorm(LEGOLS)<35){
		while ((LSvalNorm(LEGOLS)<35 || LSvalNorm(LEGOLS)>45) ||(LSvalNorm(LEGOLS2)<35 || LSvalNorm(LEGOLS2)>45)) {
			halt();
			while (LSvalNorm(LEGOLS)<35 || LSvalNorm(LEGOLS)>45) {
				if (LSvalNorm(LEGOLS)<35){
					motor[DRIVE_NE] = -40;
					motor[DRIVE_SW] = 40;
				}
			...
			halt();
			while (LSvalNorm(LEGOLS2)<35 || LSvalNorm(LEGOLS2)>45) {
				if (LSvalNorm(LEGOLS2)<35){
					motor[DRIVE_SE] = 40;
					motor[DRIVE_NW] = -40;
				}
				else{
