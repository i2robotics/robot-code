...
task main()
{
	...
	if (choiceArray[0]==1) { // if start == 1 || right
		...
		//	drive(S,20,0);
		//}
		if (nMotorEncoder[DRIVE_NW] > 4500) {//if basket 1 or 2
			wheelEnc_3 = nMotorEncoder[DRIVE_NW];
			while ((nMotorEncoder[DRIVE_NW] - wheelEnc_3) > -365) {//drive back
				drive(S,20,0);
			}
		}
		else {//if basket 3 or 4
			while (nMotorEncoder[DRIVE_NW] - wheelEnc_3< 100) {//drive further
				drive(N,20,0);
			}
		}
