...
void drive(int d, byte s = 100, short t = 1000) {   //3 imputs: direction, speed, and time to wait.
	...
	int ne =0;							//Values that will eventually become motor values.
	int se =0;
	int nw =0;
	int sw =0;
	...
	if (d >= 500) {						// TLAC
		...
		*/
		...
		case 410: //nw
			...
			//
		default:
			x = cos(-d+90.0);			//Trigonometry to travel at specified angle.
			y = sin(-d+90.0);
...
//writeDebugStreamLine(":%d,%d,%d,%d",DRIVE_NE,DRIVE_SE,DRIVE_NW,DRIVE_SW);//For debuging purposes.
}
