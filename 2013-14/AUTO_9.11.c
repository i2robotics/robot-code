...
task main()
{
	...
	if (choiceArray[0]==1) { // if start == 1 || right
		...
		//	drive(S,20,0);
		//}
		if (nMotorEncoder[DRIVE_NW] > 3600) {//if basket 1 or 2
			//wait1Msec(6000);
			//while ((nMotorEncoder[DRIVE_NW] - wheelEnc_3) > -100) {//drive back
			//	drive(S,20,0);
			//}
		}
		else {//if basket 3 or 4
			while (nMotorEncoder[DRIVE_NW] - wheelEnc_3< 500) {//drive further
				drive(N,20,0);
			}
		}
		...
		//	drive(S,20,0);
		//}
		if (nMotorEncoder[DRIVE_NW] < -4700) {//if 3 or 4
			/*writeDebugStreamLine("h%d", nMotorEncoder[DRIVE_NW]);
			writeDebugStreamLine("o%d", wheelEnc_3);
			wait1Msec(6000);
			writeDebugStreamLine("h%d", nMotorEncoder[DRIVE_NW]);*/

			while ((nMotorEncoder[DRIVE_NW] - wheelEnc_3) < 0) {
				//writeDebugStreamLine("a%d", (nMotorEncoder[DRIVE_NW] - wheelEnc_3));
				drive(N,20,0);//go back
			}
			//writeDebugStreamLine("n%d", nMotorEncoder[DRIVE_NW]);
		}
		else{//if 1 or 2
			//wait1Msec(6000);
			while ((nMotorEncoder[DRIVE_NW] - wheelEnc_3) > -300) {
				drive(S,30,0);//go further
			}
		}
	...
	if (choiceArray[1]==0) { //if end == left 0
		if (choiceArray[0]==1){ // if started right
			...
			//writeDebugStreamLine("%d", wheelEnc_2);
		...
		if (choiceArray[0]==1){// if started right
			wheelEnc_2 = wheelEnc;//wheelEnc as at the end of start
			writeDebugStreamLine("%d", wheelEnc_2);
			while (wheelEnc_2>400) {//was 1025, then 850
				writeDebugStreamLine("%d", wheelEnc_2);
				drive(S,speed,0);
				wheelEnc_2 = wheelEnc + nMotorEncoder[DRIVE_NW];//= encVal at end of start + since this while started
				if (time1[T1]>4000) {//safety
